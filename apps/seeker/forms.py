...
class SeekerRegistrationForm(UserCreationForm):
    ...
    first_name = forms.CharField(
        ...
        widget=forms.TextInput(attrs={
            'class': 'form-control',
            'maxlength': '50', 'autocomplete': 'given-name', 'placeholder': 'First name'
            }),
        )
    ...
    last_name = forms.CharField(
        max_length=50,
        widget=forms.TextInput(attrs={'class': 'form-control', 'maxlength': '50', 'autocomplete': 'family-name', 'placeholder': 'Last name'}),
    )
    ...
    email = forms.EmailField(
        max_length=254,
        widget=forms.EmailInput(attrs={'class': 'form-control', 'maxlength': '254', 'autocomplete': 'email', 'placeholder': 'you@example.com'}),
    )

    email.label = mark_safe('<i class="bi bi-envelope"></i> Email')

    password1 = forms.CharField(
        widget=forms.PasswordInput(attrs={'class': 'form-control', 'autocomplete': 'new-password', 'placeholder': 'At least 8 characters'}),
    )

    password1.label = mark_safe('<i class="bi bi-file-lock2"></i> Create Password')

    password2 = forms.CharField(
        widget=forms.PasswordInput(attrs={'class': 'form-control', 'autocomplete': 'new-password', 'placeholder': 'Re-enter your password'}),
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Do not jump past name fields or scroll the mobile page on load.
...
class SeekerLoginForm(forms.Form):
    ...
    password = forms.CharField(
        ...
        widget=forms.PasswordInput(attrs={
            'class': 'form-control', 'autocomplete': 'current-password', 'placeholder': 'Enter your password'
            })
        )
...
class SeekerCsvUploadForm(forms.Form):
    csv_file = forms.FileField(label='Upload CSV', widget=forms.ClearableFileInput(attrs={'accept': '.csv,text/csv'}))
...
class AddSeekerCSVContactInformationForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for name in ('first_name', 'last_name', 'company', 'position', 'mini_bio'):
            self.fields[name].max_length = 255
            self.fields[name].validators.append(MaxLengthValidator(255))
